...
class WPJAM_Related_Posts extends WPJAM_Option_Model{
	public static function get_fields(){
		...
		$fields		= [
			...
			'thumb_set'	=> ['title'=>'列表内容',	'type'=>'fieldset',	'fields'=>[
				...
				'_view'		=> ['type'=>'view',		'show_if'=>$show_if,	'value'=>'如勾选之后缩略图不显示，请到「<a href="'.admin_url('page=wpjam-thumbnail').'">缩略图设置</a>」勾选「无需修改主题，程序自动使用 WPJAM 的缩略图设置」。']
			]],
	...
	public static function filter_post_json($post_json){
		if($post_json['id'] == get_queried_object_id()){
			$post_json['related']	= wpjam_get_related_posts($post_json['id'], self::get_args(), $parse_for_json=true);
		}
