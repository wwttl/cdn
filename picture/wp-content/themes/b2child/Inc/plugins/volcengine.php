...
if (!empty(lmy_get_option('g_imgx_fieldset')['g_imgx'])) {

    require_once 'volcengine-imagex/vendor/autoload.php';

    function imagex_get_client()
    {
        $imagex_client = Volc\Service\ImageX::getInstance($region = lmy_get_option('g_imgx_fieldset')['g_imgx_region']);
        $imagex_client->setAccessKey(lmy_get_option('g_imgx_fieldset')['g_imgx_accesskey']);
        $imagex_client->setSecretKey(lmy_get_option('g_imgx_fieldset')['g_imgx_secretkey']);
    ...
    function imagex_upload($object, $file)
    {
        ...
        if (@file_exists($file)) {
            ...
            $params = array();
            $params["ServiceId"] = lmy_get_option('g_imgx_fieldset')['g_imgx_serviceid'];
            $params['UploadNum'] = 1;
    ...
    function imagex_delete_remote_file($file)
    {
        ...
        $client->deleteImages(lmy_get_option('g_imgx_fieldset')['g_imgx_serviceid'], array($del_file_path));
    ...
    {
        $upload_path = '';
        $upload_url_path = lmy_get_option('g_imgx_fieldset')['g_imgx_url'];
    ...
    function imagex_setting_content_ci($content)
    {
        ...
        if (!empty($images) && isset($images[1])) {
            foreach ($images[1] as $item) {
                $content = str_replace($item, $item . lmy_get_option('g_imgx_fieldset')['g_imgx_tmp'], $content);
            }
        }
        return $content;
    }
    ...
    function imagex_setting_post_thumbnail_ci($html, $post_id, $post_image_id)
    {
        if (has_post_thumbnail()) {
            ...
            if (!empty($images) && isset($images[1])) {
                foreach ($images[1] as $item) {
                    $html = str_replace($item, $item . lmy_get_option('g_imgx_fieldset')['g_imgx_tmp'], $html);
                }
            }
        }
        return $html;
